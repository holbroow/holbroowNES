...
int main(int argc, char* argv[]) {
    ...
    // Initialise Display
    InitWindow(NES_WIDTH, NES_HEIGHT, "holbroowNes test");

    // Create an Image from the framebuffer
    ...
    while (cpu->running) {
        if (cpu->cycles_left > 0) {
            ...
            // Update Display (need to complete)
            BeginDrawing();
            ...
            UpdateTexture(framebufferTexture, ppu->framebuffer);

            // // Define source and destination rectangles
            // Rectangle sourceRec = { 0.0f, 0.0f, (float)NES_WIDTH, (float)NES_HEIGHT };
            // Rectangle destRec = { 0.0f, 0.0f, (float)NES_WIDTH, (float)NES_HEIGHT };
            // Vector2 origin = { 0.0f, 0.0f };
            // float rotation = 0.0f;

            // // Draw the texture scaled up
            // DrawTexturePro(framebufferTexture, sourceRec, destRec, origin, rotation, WHITE);
